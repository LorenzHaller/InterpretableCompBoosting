...
params_svm <- makeParamSet(makeIntegerParam("cost", lower = 0.0000001, upper = 10000000),
                           makeNumericParam("gamma", lower = 0.00001, upper = 10000000))

res_svm2 = tuneParams("regr.svm", task = tsk, resampling = rdesc_tune,
                      par.set = params_svm, control = ctrl)

regr.svm2 = setHyperPars(makeLearner("regr.svm"), mstop = res_svm2$x$cost, 
                         nu = res_svm2$gamma)
