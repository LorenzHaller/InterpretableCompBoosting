...
#' @export
makeRLearner.classif.xgboost.mod = function() {
  makeRLearnerClassif(
    cl = "classif.xgboost.mod",
    package = "xgboost",
...
#' @export
trainLearner.classif.xgboost.mod = function(.learner, .task, .subset, .weights = NULL, ...) {
  ...
  task.data = createDummyFeatures(task.data, target = "net.rent")
  target.data = task.data[, 1]
  task.data = task.data[, -1]
  
  label = match(as.character(target.data), td$class.levels) - 1
  ...
  label = nlvls - as.integer(target.data)
  parlist$data = xgboost::xgb.DMatrix(data = data.matrix(task.data), label = label)
...
#' @export
predictLearner.classif.xgboost.mod = function(.learner, .model, .newdata, ...) {
  ...
  p = predict(m, newdata = data.matrix(createDummyFeatures(.newdata)), ...)
...
#' @export
getFeatureImportanceLearner.classif.xgboost.mod = function(.learner, .model, ...) {
